...
- **`RiftSentry-win-x64-self-contained.zip`** — single `.exe`, no .NET install. Built as a compressed single-file app with full globalization support so the WPF settings UI works correctly on packaged builds.
